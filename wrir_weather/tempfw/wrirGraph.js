...
function fetchData(startDate,endDate,siteId) {
    ...
    return new Promise(function(resolve,reject){
        ...
        postData.append('tbl','AirTemperature'); // <----- change this to change the targeted table
...
function updateGraph(startDate,endDate,siteId) {
    ...
    fetchData(startDate,endDate,siteId).then(function(results){
        ...
        var area = d3.area()
            ...
            .y0(height)
            .y1(function(d, i) { return y(d.temp); })
       ...
       // var linee = d3.area()
        ...
        var area2 = d3.area()
            ...
            .y0(height2)
            .y1(function(d,i) { return y2(d.temp); });
      ...
      //  var linee2 = d3.area()
       //     .curve(d3.curveMonotoneX)
        ...
        var obs =    results.tblAirTemperature;
		console.log(obs);
        var datese = obs.map(a => a.Samp_Datee.date);
        var dates =  datese.map(format)
        var temp =   obs.map(a => a.AirTemperature_C);
        //var windg =  obs.map(a => a.WindGusts_m_s);
////current parameter to display
  var now= String(temp[temp.length-1]);
var noww= 'Current Temperature: '+ now.substr(0,4)+' C';
var nowg= String(temp[temp.length-1]*1.8+32);
var nowwg= 'Current Temperature: '+ nowg.substr(0,4)+' F';
        ...
        y.domain(d3.extent(data.temp));
        x2.domain(x.domain());
        y2.domain(y.domain());
        y3.domain(d3.extent(data.temp));
	...
	//	 .text("Wind Gust");       
	   ...
	   var supplanting = false;
        ...
        document.getElementById("siteIdSelect").onchange = function(event) {
		 ...
		 document.getElementById('windnow').innerHTML = noww
		 document.getElementById('windnowG').innerHTML = nowwg
